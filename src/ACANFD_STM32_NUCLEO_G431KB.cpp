...
//--------------------------------------------------------------------------------------------------
//    THIS FILE IS SPECIFIC TO NUCLEO-G431KB
//--------------------------------------------------------------------------------------------------

#ifdef ARDUINO_NUCLEO_G431KB
...
static GPIO_TypeDef * GPIOAddressArray [2] = {GPIOA, GPIOB} ;
...
//---------------------------------------------- Select CAN clock
   ...
   RCC->CCIPR |=  (2 << RCC_CCIPR_FDCANSEL_Pos) ; // Select PCLK clock as CAN clock (170 MHz)
...
//------------------------------------------------------ Select fdcan_tq_clk CAN clock
  FDCAN_CONFIG->CKDIV = 1 ; // fdcan_tq_clk = fdcan_ker_ck / 2 = 85 MHz
...
//-------------------- Allocate Standard ID Filters (0 ... 28 elements -> 0 ... 28 words)
  ...
  for (uint32_t i=0 ; i<inStandardFilters.count () ; i++) {
    uint32_t * address = (uint32_t *) (SRAMCAN_BASE + 4 * i) ;
...
//-------------------- Allocate Extended ID Filters (0 ... 8 elements -> 0 ... 16 words)
  ...
  for (uint32_t i=0 ; i<inExtendedFilters.count () ; i++) {
    uint32_t * address = (uint32_t *) (SRAMCAN_BASE + 0x70 + 8 * i) ;
...
//--- Compute Tx Buffer address
  volatile uint32_t * txBufferPtr = (uint32_t *) (SRAMCAN_BASE + 0x278) ;
...
//--- Get messages
  ...
  //--- Get from FIFO 0
    ...
    //--- Compute message RAM address
      const uint32_t * address = (uint32_t *) (SRAMCAN_BASE + 0x00B0) ;
  ...
  //--- Get from FIFO 1
    ...
    //--- Compute message RAM address
      const uint32_t * address = (uint32_t *) (SRAMCAN_BASE + 0x0188) ;
      address += readIndex * WORD_COUNT_FOR_PAYLOAD_64_BYTES ;
    //--- Get message
      getMessageFrom (address, message) ;
    //--- Clear receive flag
